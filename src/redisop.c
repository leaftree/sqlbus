...
int sqlbus_write_to_redis(HSQLBUS handle)
{
	...
	if(handle->send_channel == NULL || handle->response_string == NULL)
	{
	...
	reply = redisCommand(handle->redis, "%s %s %s",
			handle->priority==PRI_R?R_key:L_key, handle->send_channel, handle->response_string);
...
int sqlbus_write_back_to_redis(HSQLBUS handle)
{
	...
	if(handle->recv_channel == NULL || handle->request_string == NULL) {
		return(RETURN_FAILURE);
	}

	reply = redisCommand(handle->redis, "%s %s %s", L_key, handle->recv_channel, handle->request_string);
	if(reply == NULL || reply->type != REDIS_REPLY_INTEGER)
	{
		if(reply && reply->str) {
...
int sqlbus_recv_from_redis(HSQLBUS handle)
{
	...
	if(reply == NULL || reply->type != REDIS_REPLY_ARRAY || reply->elements != 2)
	{
		if(reply && reply->str) {
			...
			sprintf(handle->errstr, "[redis client] No request message");
		}
		freeReplyObject(reply);
		return(RETURN_FAILURE);
	}

	handle->request_string = xStrdup(reply->element[1]->str);
...
redisContext *redis_connection(char *host, unsigned short int port, unsigned int timeo)
{
	...
	snprintf(hostname, sizeof(hostname), "%s", host!=NULL ? host : defaultHost);
...
int redis_expire(redisContext *redis, const char *key, unsigned int timeo)
{
	...
	if(redis)
	{
		reply = redisCommand(redis, "EXPIRE %s %d", key, timeo);
		if(reply == NULL) {
				retval = RETURN_FAILURE;
		}
		else
